...
const PS = require("../../models/roles/PS");

// searched drugs
const searched = asyncHandler(async (req, res) => {
  const { searchedDrug,id } = req.body;
const perm = await PS.findOne({ROLES:id})
// console.log(perm)
if(perm.View){  const options = {
  method: "GET",
...
const registerNewdrug = asyncHandler(async (req, res) => {
  const { batchNumber, NafdacNumber,DrugName, expirydate, Druginflow, id } = req.body;
const perm = await PS.findOne({ROLES:id})
// console.log(perm.Add)
if(perm.Add){  var DrugNamelowercased = DrugName.toLowerCase();
